...
impl Position {
    ...
    fn generate_moves_internal<const IS_WHITE: bool>(&self) -> MoveList {
        ...
        let checkers = (check_mask & self.color_bb(color.opposite())).pop_count();
        ...
        for src in unpinned_knights {
            ...
            self.add_normal_moves(&mut move_list, src, legal)
        }
    ...
    #[inline(always)]
    fn add_normal_moves(&self, move_list: &mut MoveList, src: u8, legal_targets: Bitboard) {
        for dst in legal_targets {
            let is_capture = self.color_bb(self.active_color.opposite()).get_bit(dst);
            move_list.push(Move::new_normal(src, dst, is_capture));
        }
    }
    ...
    fn generate_quiet_pawn_moves(&self, move_list: &mut MoveList, src: u8, valid_mask: Bitboard) {
        ...
        if !self.all_occupancies.get_bit(dst) {
            ...
            if END_RANKS.get_bit(dst) {
                ...
                // Normal move
                if valid_mask.get_bit(dst) {
                    move_list.push(Move::new_normal(src, dst, false));
                }
                ...
                if init_rank.get_bit(src) && ((!self.all_occupancies) & valid_mask).get_bit(double_push_sq) {
                    move_list.push(Move::new(src, double_push_sq, DoublePush));
                }
            }
        }
    }
    ...
    fn generate_pawn_captures<const HAS_ENPASSANT: bool>(&self, move_list: &mut MoveList, src: u8, check_mask: Bitboard, pin_mask: Bitboard) {
        ...
        for dst in captures {
            if !promoting {
                move_list.push(Move::new_normal(src, dst, true));
            } else {
        ...
        for enp_sq in captures {
            ...
            if !pin_mask.get_bit(captured) {
                ...
                move_list.push(Move::new(src, enp_sq, Enpassant));
            }
        }
    }
    ...
    fn generate_king_moves<const GEN_CASTLING: bool>(&self, move_list: &mut MoveList) {
        ...
        self.add_normal_moves(move_list, king_pos, legal);
        ...
        match color {
            Color::White => {
                if self.castling_ability.is_side_available(WhiteKingSide) {
                    let none_attacked = WhiteKingSide.attacked_mask() & attacked;
                    let between_open = WhiteKingSide.open_mask() & self.all_occupancies;
                    if (none_attacked | between_open).is_empty() {
                        move_list.push(Move::new(e1 as u8, g1 as u8, CastleKingSide));
                    }
                }
                if self.castling_ability.is_side_available(WhiteQueenSide) {
                    let none_attacked = WhiteQueenSide.attacked_mask() & attacked;
                    let between_open = WhiteQueenSide.open_mask() & self.all_occupancies;
                    if (none_attacked | between_open).is_empty() {
                        move_list.push(Move::new(e1 as u8, c1 as u8, CastleQueenSide));
                    }
                }
            },
            Color::Black => {
                if self.castling_ability.is_side_available(BlackKingSide) {
                    let none_attacked = BlackKingSide.attacked_mask() & attacked;
                    let between_open = BlackKingSide.open_mask() & self.all_occupancies;
                    if (none_attacked | between_open).is_empty() {
                        move_list.push(Move::new(e8 as u8, g8 as u8, CastleKingSide));
                    }
                }
                if self.castling_ability.is_side_available(BlackQueenSide) {
                    let none_attacked = BlackQueenSide.attacked_mask() & attacked;
                    let between_open = BlackQueenSide.open_mask() &self.all_occupancies;
                    if (none_attacked | between_open).is_empty() {
                        move_list.push(Move::new(e8 as u8, c8 as u8, CastleQueenSide));
                    }
                }
            }
        }
