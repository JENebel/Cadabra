...
impl Search {
    ...
    pub fn start(&self, pos: Position, meta: SearchArgs, print: bool) -> SearchStats {
        ...
        let best_move = result.best_move.unwrap_or(pos.generate_moves().next().unwrap()); // Just take first move if search didn't provide one
...
pub fn negamax<const IS_MASTER: bool>(pos: &Position, mut alpha: i16, mut beta: i16, mut depth: u8, ply: u8, context: &mut SearchContext) -> i16 {
    ...
    if depth == 0 || ply >= MAX_DEPTH as u8 {
        ...
        return quiescence(pos, alpha, beta, ply, context, CONST_EVALUATOR);
    };
    ...
    let static_eval = pos.evaluate(CONST_EVALUATOR);
